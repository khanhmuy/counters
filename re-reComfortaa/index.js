...
socket.onmessage = event => {
  try {
    let data = JSON.parse(event.data), menu = data.menu.bm.metadata, stats = data.menu.bm.stats, hits = data.gameplay.hits, mods = data.menu.mods.str;
    ...
    if (hits.sliderBreaks != sb.innerHTML) {
      $sb.update(hits.sliderBreaks);
      sb.classList.remove('click');
      reflow(sb);
